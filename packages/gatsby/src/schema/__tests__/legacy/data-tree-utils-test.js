...
describe(`[legacy] Gatsby data tree utils`, () => {
  const nodes = [
    {
      name: `The Mad Max`,
      hair: 1,
...
describe(`[legacy] Type conflicts`, () => {
  ...
  it(`Report type conflicts and its origin`, () => {
    ...
    expect(reportConflict).toBeCalled()
    expect(reportConflict).toBeCalledWith(
  ...
  it(`Doesn't report ignored fields`, () => {
    ...
    expect(reportConflict).toBeCalled()
    expect(reportConflict).toBeCalledWith(`Conflict_3.other`, expect.any(Array))
    expect(reportConflict).not.toBeCalledWith(`Conflict_3.stringOrNumber`)
  })
})
