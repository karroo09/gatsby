...
type definitions to Gatsby in the `createSchemaCustomization` API with the
[`createTypes`](/docs/actions/#createTypes) action.
...
> [`createSchemaCustomization` API](/docs/node-apis/#createSchemaCustomization).
> Previously, we have recommended to use the `sourceNodes` API -- this is now
> deprecated.
